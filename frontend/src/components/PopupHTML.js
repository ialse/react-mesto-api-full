...
export const InputEditAvatar = memo(({ onChange, error, avatar }) => {
    return (
        <label className="popup__field">
            <input type="url" className="popup__input popup__input_link" id="link-input-avatar" name="link"
                placeholder="Ссылка на картинку для аватара" onChange={onChange} value={avatar} />
        ...
        <>
            <label className="popup__field">
                <input type="text" className="popup__input popup__input_place" id="title-input" name="name" placeholder="Название"
                    minLength="1" maxLength="30" onChange={onChange} value={place} />
            ...
            <label className="popup__field">
                <input type="url" className="popup__input popup__input_link" id="link-input-card" name="link"
                    placeholder="Ссылка на картинку" onChange={onChange} value={link} />
                <span className="popup__error popup__error_visible" id="link-input-error">
                    {(!!Object.keys(error).length && error.link.required && 'Заполните это поле.') ||
                        (!!Object.keys(error).length && error.link.http && 'Введите URL.')}
                </span>
